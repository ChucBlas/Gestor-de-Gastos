...
use tauri::command;
use crate::db::get_db;
use crate::models::{PeriodBalance, CategorySummary, MonthlyData, DashboardSummary, Transaction};

#[command]
pub fn get_period_balance(date_from: String, date_to: String, account_id: Option<i32>) -> Result<PeriodBalance, String> {
    ...
    Ok(PeriodBalance { total_income: income, total_expense: expense, net: income - expense })
}

#[command]
pub fn get_category_summary(date_from: String, date_to: String, account_id: Option<i32>) -> Result<Vec<CategorySummary>, String> {
    let db = get_db().lock().map_err(|e| e.to_string())?;
    ...
    let rows = stmt.query_map(params![date_from, date_to, account_id], |row| {
        Ok(CategorySummary {
...
pub fn get_monthly_data(year: i32, account_id: Option<i32>) -> Result<Vec<MonthlyData>, String> {
    ...
    let db_rows = stmt.query_map(params![format!("{}%", year_str), account_id], |row| {
        Ok(MonthlyData {
            month: row.get(0)?,
            income: row.get(1)?,
            expense: row.get(2)?,
...
pub fn get_dashboard_summary() -> Result<DashboardSummary, String> {
    ...
    let total_balance: f64 = db.query_row(
    ...
    let month_income: f64 = db.query_row(
    ...
    let month_expense: f64 = db.query_row(
